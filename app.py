...
def guardar_registro(consultas, controles, mensajes, llamadas, canto, consultantes_texto):
    ...
    try:
        consultas_val = int(consultas or 0)
        controles_val = int(controles or 0)
        mensajes_val = int(mensajes or 0)
        llamadas_val = int(llamadas or 0)
        canto_val = float(canto or 0.0)
    ...
    try:
        ...
        if not getattr(response, "data", None):
            st.error("❌ No se pudo guardar el registro. Revisa el esquema en Supabase.")
            return
        ...
        if consultantes_list:
            ...
            try:
                supabase.table("consultantes").insert(payload).execute()
            except Exception as e:
                st.error(f"⚠️ Error al guardar consultantes: {e}")

        st.success(f"✅ Registro guardado correctamente ({fecha})")
        ...
        if not st.session_state.is_admin:
            with st.form("form_registro"):
                ...
                consultantes_texto = st.text_area(
                    "👥 Consultantes (nombre, teléfono, etc.)",
